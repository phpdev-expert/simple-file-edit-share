...
1. Create a **Postgres** database (e.g. [Neon](https://neon.tech) free tier) and copy
   its connection string.
2. On [Render](https://render.com): **New → Blueprint**, point it at this repo. Render
   reads `render.yaml` and provisions a Docker web service.
3. Set env vars on the service:
   - `DATABASE_URL` → your Postgres connection string (for persistence across restarts)
   - `SESSION_SECRET` → auto-generated by the blueprint (or set your own)
   - `COOKIE_SECURE` → `1` (already set)
4. Deploy. The health check is `/api/health`. Seeding runs automatically on startup.

> Without `DATABASE_URL`, the app still runs on SQLite but data is ephemeral on
> Render's filesystem. Use Postgres for a persistent demo.
