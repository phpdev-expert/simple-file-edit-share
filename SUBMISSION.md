...
| Automated tests | `backend/tests/` (35 tests, `pytest`) |
...
- **Comments & suggestions**: anchored to selected text; accept a suggestion to
  rewrite the doc, or resolve/dismiss.
...
- 35 passing automated tests + AI-driven browser QA.
...
- **Inline comment highlights** — comments/suggestions anchor to the quoted text in a
  side panel rather than painting a highlight into the document body.
...
2. **Inline comment highlights** anchored with a persisted mark in the document.
...
pip install -r requirements.txt && pytest        # 35 pass
